...
class Post(db.Model):
    __tablename__ = 'post'
    id = db.Column(db.Integer, primary_key=True)
    recruiter_id = db.Column(db.Integer, db.ForeignKey('recruiter.user_id', ondelete='CASCADE'), nullable=False)
    ...
    status = db.Column(db.Enum('ACTIVE', 'OVERDUE', 'CLOSED', 'PINNED', 'BLOCKED'), default='ACTIVE')
...
class Application(db.Model):
    ...
    status = db.Column(db.Enum('RECEIVED', 'INTERVIEW', 'APPROVED', 'REJECT'), default='RECEIVED')
    cover_letter = db.Column(db.Text)
    __table_args__ = (db.UniqueConstraint('cv_id', 'post_id', name='uq_app_cv_post'),)
...
class Notification(db.Model):
    ...
    type = db.Column(db.Enum('APPLICATION_STATUS_CHANGED', 'INTERVIEW_INVITATION', 'NEW_APPLICATION', 'ACCOUNT_APPROVED', 'POST_BLOCKED'))
    created_at = db.Column(db.DateTime, default=db.func.now())
